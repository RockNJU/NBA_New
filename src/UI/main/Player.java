...
public class Player extends JPanel {
	...
	String[] playerTotaltitle={" 序号  "," 球员名称  "," 所属球队  "," 参赛场数  "," 先发场数  ",
	...
	String[] playerAvgtitle={" 序号  "," 球员名称  "," 所属球队  "," 参赛场数  "," 先发场数  ",
			" 平均篮板数  "," 平均助攻数  "," 在场时间  "," 投篮命中率  "," 三分命中率  "," 罚球命中率  ",
			" 平均进攻数  "," 平均防守数  "," 平均抢断数  "," 平均盖帽数  "," 平均失误数  "," 平均犯规数  "," 平均得分  "," 效率  ",
			" GmSc效率值  "," 真实命中率  "," 投篮效率  "," 篮板率  "," 进攻篮板率  "," 防守篮板率  ",
			" 助攻率  "," 抢断率  "," 盖帽率  "," 失误率  "," 使用率  "};
	String[] playerinfo={" 序号  "," 姓名  "," 球号  "," 位置  "," 身高  "," 体重  "," 出生日期  "," 年龄  "," 球龄  "," 毕业院校  "};
	...
	public Player() {
		...
		playerseason1 = new JComboBox();
		playerseason1.setFont(new Font("微软雅黑", Font.PLAIN, 12));
		playerseason1.setToolTipText("赛季");
		
		
		
		
		playerseason1.setModel(new DefaultComboBoxModel(seasons));
		playerseason1.setEditable(true);
		playerseason1.setBounds(422,79, 117, 30);
		add(playerseason1);
		playerseason1.setVisible(true);
*/
	...
	private Object[][] getTotaldata(ArrayList<PlayerSeasonDataVO> da){
		...
		if(da==null){
			...
			for(int i=0;i<da.size();i++){						
				...
				re[i][7]=da.get(i).getTime();
				re[i][8]=da.get(i).getShootPercentage();
				re[i][9]=da.get(i).getT_shootPercentage();
				re[i][10]=da.get(i).getFreeThrowPercentage();
				...
				re[i][18]=da.get(i).getEfficiency();
				re[i][19]=da.get(i).getGmSc();
				re[i][20]=da.get(i).getRealShootPercentage();				
				re[i][21]=da.get(i).getShootEfficiency();
				re[i][22]=da.get(i).getReboundEfficiency();
				re[i][23]=da.get(i).getOffensiveReboundEff();
				re[i][24]=da.get(i).getDefenseReboundEff();
				re[i][25]=da.get(i).getAssistEfficiency();
